...
class CollectionHrefMappingsMixin(storage.Collection):
...
class Collection(CollectionHrefMappingsMixin, storage.Collection):
        ...
        if len(attributes) == 2:
            ...
            def info_listener(path, datetime, key, value, extra):
                if key == "name":
                    ...
                    extra._set_meta_key(
                        "ICAL:calendar-color", value, update_decsync=False
                    )
                else:
            ...
            def resources_listener(path, datetime, key, value, extra):
                ...
                if value is None:
                    ...
                    item = radicale_item.Item(
                        collection=extra, vobject_item=vobject_item, uid=uid
                    )
                    item.prepare()
    ...
    def upload(self, href, item, update_decsync=True):
        item = super().upload(href, item)
        if update_decsync:
            ...
            if tag == "VCALENDAR":
                ...
                component_name = item[0].component_name
            ...
            self.set_href(item[0].uid, href)
            self.decsync.set_entry(
                ["resources", item[0].uid], None, item[0].serialize()
            )
        return item
...
class Storage(storage.Storage):
    ...
    def __init__(self, configuration):
        ...
        try:
            self.decsync_dir = os.path.expanduser(
                configuration.get("storage", "decsync_dir")
            )
        except KeyError:
            self.decsync_dir = ""
    ...
    def discover(
        ...
        path,
        depth="0",
        child_context_manager=(lambda path, href=None: contextlib.ExitStack()),
        user_groups=set(),
    ):
